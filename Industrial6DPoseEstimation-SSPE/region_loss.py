...
def build_targets(pred_corners, target, anchors, num_anchors, num_classes, nH, nW, noobject_scale, object_scale,
                  sil_thresh, seen):
    nB = target.size(0)
    nA = num_anchors
    nC = num_classes
    ...
    for b in range(nB):
        ...
        for t in range(50):
            cur_confs = cur_confs.view_as(torch.zeros(nAnchors))
            ...
            cur_gt_corners = torch.FloatTensor(
                [gx0 / nW, gy0 / nH, gx1 / nW, gy1 / nH, gx2 / nW, gy2 / nH, gx3 / nW, gy3 / nH, gx4 / nW, gy4 / nH,
                 gx5 / nW, gy5 / nH, gx6 / nW, gy6 / nH, gx7 / nW, gy7 / nH, gx8 / nW, gy8 / nH]).repeat(nAnchors,
                                                                                                         1).t()  # 16 x nAnchors
            cur_confs = torch.max(cur_confs, corner_confidences9(cur_pred_corners, cur_gt_corners)).view_as(conf_mask[
    ...
    for b in range(nB):
        for t in range(50):
            ...
            gt_box = [gx0 / nW, gy0 / nH, gx1 / nW, gy1 / nH, gx2 / nW, gy2 / nH, gx3 / nW, gy3 / nH, gx4 / nW,
                      gy4 / nH, gx5 / nW, gy5 / nH, gx6 / nW, gy6 / nH, gx7 / nW, gy7 / nH, gx8 / nW, gy8 / nH]
...
class RegionLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        nGT, nCorrect, coord_mask, conf_mask, cls_mask, tx0, tx1, tx2, tx3, tx4, tx5, tx6, tx7, tx8, ty0, ty1, ty2, ty3, ty4, ty5, ty6, ty7, ty8, tconf, tcls = \
            build_targets(pred_corners, target.data, self.anchors, nA, nC, nH, nW, self.noobject_scale,
        ...
        if False:
            ...
            self.seen, nGT, nCorrect, nProposals, loss_x.data[0], loss_y.data[0], loss_conf.data[0], loss_cls.data[0],
            loss.data[0]))
        else:
            print('%d: nGT %d, recall %d, proposals %d, loss: x %f, y %f, conf %f, total %f' % (
            self.seen, nGT, nCorrect, nProposals, loss_x.data.item(), loss_y.data.item(), loss_conf.data.item(),
            loss.data.item()))
